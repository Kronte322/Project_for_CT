...
class MapGenerator:
  ...
  def IsItWallForDoor(self, position: tuple):
    if self.GetTile(position) in [kWall]:
      if self.GetTile((position[0], position[1] + 1)) in [kWall, kDoor] and self.GetTile((position[0], position[1] - 1)) in [kWall, kDoor]:
        if self.GetTile((position[0] - 1, position[1])) in [kEmpty] and self.GetTile((position[0] + 1, position[1])) in [kFloor]:
          return True
        if self.GetTile((position[0] - 1, position[1])) in [kFloor] and self.GetTile((position[0] + 1, position[1])) in [kEmpty]:
          return True
      if self.GetTile((position[0] + 1, position[1])) in [kWall, kDoor] and self.GetTile((position[0] - 1, position[1])) in [kWall, kDoor]:
        if self.GetTile((position[0], position[1] - 1)) in [kEmpty] and self.GetTile((position[0], position[1] + 1)) in [kFloor]:
          return True
        if self.GetTile((position[0], position[1] - 1)) in [kFloor] and self.GetTile((position[0], position[1] + 1)) in [kEmpty]:
          return True
    return False

  def IsThereCorner(self, position: tuple):
    if self.IsThereAnyTile(position, [kEmpty]):
      if self.GetTile(position) in [kWall] and self.GetTile((position[0] - 1, position[1])) in [kWall] and self.GetTile((position[0], position[1] + 1)) in [kWall]:
        return True
      if self.GetTile(position) in [kWall] and self.GetTile((position[0] - 1, position[1])) in [kWall] and self.GetTile((position[0], position[1] - 1)) in [kWall]:
        return True
      if self.GetTile(position) in [kWall] and self.GetTile((position[0] + 1, position[1])) in [kWall] and self.GetTile((position[0], position[1] + 1)) in [kWall]:
  ...
  def IsThereCutCorner(self, position: tuple):
    if self.IsThereAnyTile(position, [kEmpty]):
      if self.GetTile(position) in [kFloor] and self.GetTile((position[0] - 1, position[1])) in [kWall] and self.GetTile((position[0], position[1] + 1)) in [kWall]:
        return True
      if self.GetTile(position) in [kFloor] and self.GetTile((position[0] - 1, position[1])) in [kWall] and self.GetTile((position[0], position[1] - 1)) in [kWall]:
        return True
      if self.GetTile(position) in [kFloor] and self.GetTile((position[0] + 1, position[1])) in [kWall] and self.GetTile((position[0], position[1] + 1)) in [kWall]:
  ...
  def IsThereCutCornerAfterCreate(self, position: tuple):
    if self.IsThereAnyTile(position, [kFloor]):
      if self.GetTile(position) in [kEmpty] and self.GetTile((position[0] - 1, position[1])) in [kWall] and self.GetTile((position[0], position[1] + 1)) in [kWall]:
        return True
      if self.GetTile(position) in [kEmpty] and self.GetTile((position[0] - 1, position[1])) in [kWall] and self.GetTile((position[0], position[1] - 1)) in [kWall]:
        return True
      if self.GetTile(position) in [kEmpty] and self.GetTile((position[0] + 1, position[1])) in [kWall] and self.GetTile((position[0], position[1] + 1)) in [kWall]:
  ...
  def IsInCrossAnyTile(self, position, tiles) -> bool:
    ...
    if position[1] < len(self.main_matrix) - 1:
      if self.main_matrix[position[0]][position[1] + 1] in tiles:
        return True
    if position[0] == 0 or position[0] == len(self.main_matrix) - 1 or position[1] == 0 or position[1] == len(self.main_matrix[0]) - 1:
      return True
    return False
            ...
            if self.main_matrix[left_corner_position[0] +
              ...
              self.main_matrix[left_corner_position[0] +
                               i][left_corner_position[1] + j] = kWall
                                  ...
                                  i][left_corner_position[1] + j] == kWall and not self.IsInCrossAnyTile((left_corner_position[0] +
  ...
  def DeleteWallsOnDiagonal(self):
    for i in range(len(self.main_matrix) - 1):
      for j in range(len(self.main_matrix[0]) - 1):
        if self.main_matrix[i][j] == kWall and self.main_matrix[i + 1][j] == kFloor and self.main_matrix[i][j + 1] == kFloor and self.main_matrix[i + 1][j + 1] == kWall:
          self.main_matrix[i + 1][j] = kWall
          self.main_matrix[i][j + 1] = kWall
        elif self.main_matrix[i][j] == kFloor and self.main_matrix[i + 1][j] == kWall and self.main_matrix[i][j + 1] == kWall and self.main_matrix[i + 1][j + 1] == kFloor:
  ...
  def OneWidthPathsVertical(self):
    for i in range(len(self.main_matrix)):
      for j in range(1, len(self.main_matrix[0]) - 1):
        if self.main_matrix[i][j] == kEmpty and self.main_matrix[i][j - 1] == kWall and self.main_matrix[i][j + 1] == kWall:
  ...
  def OneWidthPathsHorizontal(self):
    for i in range(1, len(self.main_matrix) - 1):
      for j in range(len(self.main_matrix[0])):
        if self.main_matrix[i][j] == kEmpty and self.main_matrix[i - 1][j] == kWall and self.main_matrix[i + 1][j] == kWall:
          self.main_matrix[i][j] = kOneWidthPath
  ...
  def TwoWidthPathsVertical(self):
    for i in range(len(self.main_matrix)):
      for j in range(1, len(self.main_matrix[0]) - 2):
        if self.main_matrix[i][j] == kEmpty and self.main_matrix[i][j - 1] == kWall and self.main_matrix[i][j + 1] == kEmpty and self.main_matrix[i][j + 2] == kWall:
          self.main_matrix[i][j] = kOneWidthPath
  ...
  def TwoWidthPathsHorizontal(self):
    for i in range(1, len(self.main_matrix) - 2):
      for j in range(len(self.main_matrix[0])):
        if self.main_matrix[i][j] == kEmpty and self.main_matrix[i - 1][j] == kWall and self.main_matrix[i + 1][j] == kEmpty and self.main_matrix[i + 2][j] == kWall:
          self.main_matrix[i][j] = kOneWidthPath
  ...
  def IsThereDoorBetweenRooms(self, position: tuple):
    if (self.GetTile(position) in [kEmpty] and self.GetTile((position[0] - 1, position[1])) in [kWall] and self.GetTile((position[0] + 1, position[1])) in [kWall]) or (self.GetTile(position) in [kEmpty] and self.GetTile((position[0], position[1] - 1)) in [kWall] and self.GetTile((position[0], position[1] + 1)) in [kWall]):
      if self.GetTile((position[0] - 1, position[1] + 1)) in [kWall] and self.GetTile((position[0] - 1, position[1] - 1)) in [kWall]:
        if self.GetTile((position[0] + 1, position[1] + 1)) in [kWall] and self.GetTile((position[0] + 1, position[1] - 1)) in [kWall]:
          return True
    return False
...
class DFSAlgo:
        ...
        if j not in [(-1, -1), vertex]:
          if self.parents.get(vertex) != None:
            if self.map.IsItWallForDoor(j):
              if self.used.get((j[0], j[1] - 2)) != None or self.used.get((j[0], j[1] + 2)) != None or self.used.get((j[0] - 2, j[1])) != None or self.used.get((j[0] + 2, j[1])) != None:
  ...
  def FinalDFS(self, vertex, final_matrix):
    ...
    for i in self.map.GetNeighbours(vertex):
      if self.used.get(i) == None and i != self.parents.get(vertex) and self.map.GetTile(i) not in [kEmpty, kBoardOfMap]:
        self.parents[i] = vertex
