...
# import src.map_generator as mapgen
import pygame
import random
import importlib
import time
...
class DiscreteDistribution:
  def __init__(self, list_of_pairs) -> None:
    self.list_of_values = []
    interm = 0
    for i in list_of_pairs:
      self.list_of_values.append([i[0], [interm, interm + i[1] * 100]])
      interm += i[1] * 100

  def GetRandomItem(self):
    interm = random.randrange(0, 101)
    for i in self.list_of_values:
      if interm >= i[1][0] and interm < i[1][1]:
        return i[0]
...
path_generator = DiscreteDistribution(
...
class Player:
  def __init__(self):
    ...
    self.moveBox = (kSizeOfDisplay[0] // 2 - kSizeOfMoveBox[0] // 2, kSizeOfDisplay[1] // 2 - kSizeOfMoveBox[1] //
                    2, kSizeOfDisplay[0] // 2 + kSizeOfMoveBox[0] // 2, kSizeOfDisplay[1] // 2 + kSizeOfMoveBox[1] // 2)
